...
export default class EmprendimientoUpdatePage {
  ...
  estadoFirmaInput: ElementFinder = element(by.css('input#emprendimiento-estadoFirma'));
  grupoEmprendimientoSelect: ElementFinder = element(by.css('select#emprendimiento-grupoEmprendimiento'));
  ...
  async setEstadoFirmaInput(estadoFirma) {
    await this.estadoFirmaInput.sendKeys(estadoFirma);
  }

  async getEstadoFirmaInput() {
    return this.estadoFirmaInput.getAttribute('value');
  }
  ...
  async enterData() {
    ...
    await waitUntilDisplayed(this.saveButton);
    await this.setEstadoFirmaInput('estadoFirma');
    expect(await this.getEstadoFirmaInput()).to.match(/estadoFirma/);
